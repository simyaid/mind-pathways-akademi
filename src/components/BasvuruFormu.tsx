...
export const BasvuruFormu = ({ open, onClose }: Props) => {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && handleClose()}>
      <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-display text-lg leading-snug text-navy-deep">
            2026-2027 Dönemi Akademi Psikodiyalektik<br />
            Formasyona Hazırlık Programı Önbaşvuru Formu
          </DialogTitle>
        </DialogHeader>

        {submitted ? (
          <div className="py-8 text-center space-y-3">
            <p className="font-display text-xl text-navy-deep">Başvurunuz alındı.</p>
            <p className="text-sm text-muted-foreground">
              Önbaşvuru formunuz bize ulaştı. En kısa sürede sizinle iletişime geçilecektir.
            </p>
            <Button variant="amber" className="mt-4" onClick={handleClose}>Kapat</Button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-5 pt-1">
            <div className="space-y-2 text-sm text-muted-foreground leading-relaxed">
              ...
              <p>Önbaşvuru formunuz bize ulaştıktan sonra sizinle iletişime geçilecektir.</p>
            </div>
            ...
            <div className="rounded-lg border border-border bg-surface p-4 text-xs text-muted-foreground leading-relaxed">
            ...
            <div className="flex items-center gap-2">
              <Checkbox
                ...
                required
              />
              <Label htmlFor="kvkk" className="text-sm cursor-pointer">Onaylıyorum</Label>
            </div>
